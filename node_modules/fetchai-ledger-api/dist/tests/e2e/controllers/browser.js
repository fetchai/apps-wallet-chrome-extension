...
async function test_wealth() {
  ...
  _assert.Assert.assert_equal(wealth, 1000);

  _utils.logger.info('test_wealth passed');
}

function get_script(name) {
  let bundle = _fs.default.readFileSync(_path.default.join(ROOT_FP + '/bundle/vanilla.js'), 'utf8');
...
function get_driver() {
  return new _seleniumWebdriver.default.Builder().forBrowser('chrome').setChromeOptions(new _chrome.default.Options().headless().windowSize({
    width: 640,
    height: 480
  })).build();
