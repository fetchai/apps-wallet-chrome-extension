...
const [ENTITIES, ADDRESSES] = (() => {
  const ENTITIES = [];
  ENTITIES.push(new _entity.Entity(Buffer.from('19c59b0a4890383eea59539173bfca5dc78e5e99037f4ad65c93d5b777b8720e', 'hex')));
  ENTITIES.push(new _entity.Entity(Buffer.from('e1b74f6357dbdd0e03ad26afaab04071964ef1c9a0f0abf10edb060e06c890a0', 'hex')));
  const ADDRESSES = [];
  ADDRESSES.push(new _address.Address(ENTITIES[0]));
  ADDRESSES.push(new _address.Address(ENTITIES[1]));
...
describe(':ContractsApi', () => {
  ...
  test('test create', async () => {
    ...
    await api.sync([tx]);
    const contract = new _contract.Contract(_contracts2.TRANSFER_CONTRACT, ENTITIES[0], NONCE);
    ...
    expect(_axios.default).toHaveBeenCalledTimes(6);
  });
  test('test query', async () => {
    const api = new _api.LedgerApi(_helpers.LOCAL_HOST, _helpers.DEFAULT_PORT);
    const contract = new _contract.Contract(_contracts2.TRANSFER_CONTRACT, ENTITIES[0], NONCE);
    const query = await contract.query(api, 'balance', {
      address: ADDRESSES[0]
    });
  ...
  test('test _encode_json_payload', () => {
    const api = new _contracts.ContractsApi(_helpers.LOCAL_HOST, _helpers.DEFAULT_PORT);
    const args = [];
    args.push(new _address.Address('2J8wzPaBFRc2CtdRLkhRG5488HfrpkET8a5aHArmL5dLqvm7ED'));
    args.push(new _address.Address('2X5fnrS8gM92BcnLp8mUTpp8LYGQx9wxyRLo6zHokBAMHMrMgL'));
    args.push(200);
  ...
  test('test _encode_json_payload with underscore as last character of key', () => {
    const api = new _contracts.ContractsApi(_helpers.LOCAL_HOST, _helpers.DEFAULT_PORT);
    const underscore_test1_ = new _address.Address('2J8wzPaBFRc2CtdRLkhRG5488HfrpkET8a5aHArmL5dLqvm7ED');
    const underscore_test2_ = new _address.Address('2X5fnrS8gM92BcnLp8mUTpp8LYGQx9wxyRLo6zHokBAMHMrMgL');

    const encoded = api._encode_json_payload({
      underscore_test1_,
      underscore_test2_
    });
  ...
  test('test _encode_json_payload with nested object', () => {
    const api = new _contracts.ContractsApi(_helpers.LOCAL_HOST, _helpers.DEFAULT_PORT);
    const underScoreTest1_ = new _address.Address('2J8wzPaBFRc2CtdRLkhRG5488HfrpkET8a5aHArmL5dLqvm7ED');
    const underScoreTest2_ = new _address.Address('2X5fnrS8gM92BcnLp8mUTpp8LYGQx9wxyRLo6zHokBAMHMrMgL');
    const nest = {
      underScoreTest1_,
      underScoreTest2_
    };
  ...
  test('test message pack encode', async () => {
    ...
    const actual = api._encode_msgpack_payload(args); // we compare as hex
