...
describe(':Entity', () => {
  test('test generation', () => {
    const reference = new _entity.Entity();
    const other = new _entity.Entity(reference.private_key());
  ...
  test('test signing verifying cycle', () => {
    ...
    const entity = new _entity.Entity(); // sign the payload

    const sign_obj = entity.sign(digest); // verify the payload

    const verified = entity.verify(digest, sign_obj.signature);
    expect(verified).toBe(true); // create bad 64 byte sig
  ...
  test('test construction from base64', () => {
    ...
    const other = _entity.Entity.from_base64(base64_data);
  ...
  test('test invalid_construction', () => {
    expect(() => {
      ...
      new _entity.Entity(Buffer.from('123'));
  ...
  test('test loads', () => {
    ...
    const entity = _entity.Entity.loads(s);
