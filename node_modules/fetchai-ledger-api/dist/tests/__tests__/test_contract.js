"use strict";

var _crypto = require("../../fetchai/ledger/crypto");

var _contract = require("../../fetchai/ledger/contract");

var _errors = require("../../fetchai/ledger/errors");

var _helpers = require("../utils/helpers");

var _btoa = _interopRequireDefault(require("btoa"));

var _crypto2 = require("crypto");

var _transfer = require("../../contracts/transfer");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const calc_address = (owner, nonce) => {
  const hash_func = (0, _crypto2.createHash)('sha256');
  hash_func.update(owner.toBytes(), 'utf');
  hash_func.update(nonce, 'utf');
  return hash_func.digest();
};

const _compute_digest = source => {
  const hash_func = (0, _crypto2.createHash)('sha256');
  hash_func.update(source, 'ascii');
  const d = hash_func.digest();
  return new _crypto.Address(d);
};

jest.mock('fs', () => {
  const MOCK_FILE_INFO = '{"version":1,"owner":"2mhttHhKVRdY1n9BsFNHVJgHjGPvBmnA2FXKCPkJaC6TkXmaY9","source":"","nonce":"pEGxX+mjz1Y="}';
  const EXPECTED_FP = '/path/to/file';
  return {
    readFileSync: () => {
      return MOCK_FILE_INFO;
    },
    writeFileSync: (fp, json_string) => {
      expect(fp).toBe(EXPECTED_FP);
      expect(json_string).toBe(MOCK_FILE_INFO);
    }
  };
});
describe(':Test Contract', () => {
  test('test dumps and loads', () => {
    const owner = new _crypto.Entity();
    const nonce = (0, _helpers.calc_digest)('random').slice(0, 8);
    const orig = new _contract.Contract(_transfer.SIMPLE_CONTRACT, owner, nonce);
    const encoded = orig.dumps();

    const recreation = _contract.Contract.loads(encoded);

    expect(recreation).toBeInstanceOf(_contract.Contract);
    expect(orig.owner()).toMatchObject(recreation.owner());
    expect(orig.digest()).toMatchObject(recreation.digest());
    expect(orig.source()).toBe(recreation.source());
  });
  test('test dumps and loads without nonce', () => {
    const owner = new _crypto.Entity();
    const orig = new _contract.Contract(_transfer.SIMPLE_CONTRACT, owner);
    const encoded = orig.dumps();

    const recreation = _contract.Contract.loads(encoded);

    expect(recreation).toBeInstanceOf(_contract.Contract);
    expect(orig.owner()).toMatchObject(recreation.owner());
    expect(orig.digest()).toMatchObject(recreation.digest());
    expect(orig.source()).toBe(recreation.source());
  });
  test('test dump and load', () => {
    const rand_bytes = (0, _helpers.calc_digest)('rand');
    const owner = new _crypto.Entity(rand_bytes);
    const nonce = (0, _helpers.calc_digest)('random').slice(0, 8);
    const orig = new _contract.Contract(_transfer.SIMPLE_CONTRACT, owner, nonce);
    const encoded = orig.dump(_helpers.RAND_FP);

    const recreation = _contract.Contract.load(encoded);

    expect(recreation).toBeInstanceOf(_contract.Contract);
    expect(orig.owner()).toMatchObject(recreation.owner());
    expect(orig.digest()).toMatchObject(recreation.digest());
  });
  test('test owner getter and setter', () => {
    const rand_bytes = (0, _helpers.calc_digest)('rand');
    const owner = new _crypto.Entity(rand_bytes);
    const nonce = (0, _helpers.calc_digest)('random').slice(0, 8);
    const orig = new _contract.Contract(_transfer.SIMPLE_CONTRACT, owner, nonce);
    const rand_bytes_two = (0, _helpers.calc_digest)('rand2');
    const owner2 = new _crypto.Entity(rand_bytes_two); //reset owner using setter

    const actual_owner2 = orig.owner(owner2);
    expect(actual_owner2.toBytes()).toMatchObject(new _crypto.Address(owner2).toBytes());
  });
  test('test getters and setters', () => {
    const rand_bytes = (0, _helpers.calc_digest)('rand');
    const owner = new _crypto.Address(new _crypto.Entity(rand_bytes));
    const nonce = (0, _helpers.calc_digest)('random').slice(0, 8);
    const orig = new _contract.Contract(_transfer.SIMPLE_CONTRACT, owner, nonce); //check if we can get rid of this ascii bit

    const contract_string = Buffer.from(_transfer.SIMPLE_CONTRACT).toString('ascii');

    const ref_digest = _compute_digest(contract_string);

    const ref_address = new _crypto.Address(calc_address(owner, nonce));
    const ref_name = ref_digest.toBytes().toString('hex') + ref_address.toHex();
    expect(orig.nonce()).toBe((0, _btoa.default)(nonce));
    expect(orig.nonce_bytes()).toMatchObject(nonce);
    expect(orig.address().toBytes()).toMatchObject(ref_address.toBytes());
    expect(orig.name()).toBe(ref_name);
    expect(orig.encoded_source()).toBe((0, _btoa.default)(contract_string));
    expect(orig.digest()).toMatchObject(ref_digest);
    expect(orig.source()).toBe(contract_string);
  });
  test.skip('test create', () => {// create contract
    // const contract = new Contract(SIMPLE_CONTRACT)
    // const owner = new Entity()
    // Mock api for providing number of lanes and receiving create call
    //              const HOST = '127.0.0.1';
    // const PORT = 8000;
    //         const api = new LedgerApi(HOST, PORT)
    // lane_number = 2
    // api.server.num_lanes.side_effect = [lane_number]
    // Mock shard mask static method
    // dummy_shard_mask = mock.Mock()
    // mock_shard_mask.side_effect = [dummy_shard_mask]
    //   contract.create(api, owner, 1000)
    // Check shard mask gen called with contract digest address
    //  mock_shard_mask.assert_called_once_with(
    //    ['fetch.contract.state.{}'.format(contract.digest.to_hex())], lane_number)
    // Check api create method called
    //   api.contracts.create.assert_called_once_with(owner, contract, 1000, shard_mask=dummy_shard_mask)
    // create the contract
    // const orig = new Contract(SIMPLE_CONTRACT)
    // encode the contract
    //   const encoded = orig.dumps()
    // re-create the contract
    //   const recreation = Contract.loads(encoded)
    // checks
    //  expect( recreation ).toBeInstanceOf(Contract)
    //    expect(orig.owner()).toMatchObject(recreation.owner())
    // expect(orig.digest()).toMatchObject(recreation.digest())
    // expect(orig.source()).toMatchObject(recreation.source())
  }); //TODO remove skip when we have etchparser support

  test.skip('test init', () => {
    const owner = new _crypto.Entity();
    expect(() => {
      new _contract.Contract(_transfer.MULTIPLE_INITS, owner);
    }).toThrow(_errors.RunTimeError); // Test successful creation without init (to support local etch testing)
    // expect(() => {
    //    let contract = new Contract(MULTIPLE_INITS, owner)
    // }).toThrow(RunTimeError)
    // create ledgerAPI instance without calling its es6 constructor

    const api = Object.create(_contract.Contract.prototype);
    expect(() => {
      api.create(api, owner, 100);
    }).toThrow(_errors.RunTimeError);
  });
});