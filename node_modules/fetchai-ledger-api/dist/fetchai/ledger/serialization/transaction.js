...
exports.decode_transaction = exports.encode_transaction = void 0;
...
var bytearray = _interopRequireWildcard(require("./bytearray"));
...
var _identity2 = require("../crypto/identity");
...
// ********** Constants **********
// *******************************
const MAGIC = 0xa1; // a reserved byte.
...
const _map_contract_mode = payload => {
  ...
  if (payload.action()) {
    ...
    (0, _assert.default)(payload.contract_digest() != null);
...
const encode_payload = payload => {
  ...
  const num_signatures = Object.keys(payload._signers).length; // sanity check

  (0, _assert.default)(num_signatures >= 1);
  const num_extra_signatures = num_signatures > 0x40 ? num_signatures - 0x40 : 0;
  const signalled_signatures = num_signatures - (num_extra_signatures + 1);
  const has_valid_from = payload._valid_from.cmp(new _bn.BN(0)) !== 0;
  let header0 = VERSION << 5; /// ??

  header0 |= (num_transfers > 0 ? 1 : 0) << 2;
  header0 |= (num_transfers > 1 ? 1 : 0) << 1;
  header0 |= has_valid_from ? 1 : 0; // determine the mode of the contract
  ...
  if (num_transfers > 1) {
    buffer = integer.encode_integer(buffer, new _bn.BN(num_transfers - 2));
  }
  ...
  if (has_valid_from) {
    buffer = integer.encode_integer(buffer, new _bn.BN(payload.valid_from()));
  }
  ...
  if (NO_CONTRACT !== contract_mode) {
    ...
    if (shard_mask_length <= 1) {
      ...
      if (shard_mask_length < 8) {
        (0, _assert.default)(Buffer.byteLength(shard_mask_bytes) === 1);
        ...
        (0, _assert.default)(shard_mask_length <= 512);
  ...
  if (num_extra_signatures > 0) {
    buffer = integer.encode_integer(buffer, new _bn.BN(num_extra_signatures));
  } // write all the signers public keys


  for (let signer of Object.keys(payload._signers)) {
    buffer = identity.encode_identity(buffer, Buffer.from(signer, 'hex'));
  }
...
const encode_transaction = (payload, signers) => {
  ...
  let buffer = encode_payload(payload); // extract the payload buffer

  let payload_bytes = _calc_digest_utf(buffer); // append all the signatures of the signers in order
  ...
  for (let signer of Object.keys(payload._signers)) {
    ...
    for (let i = 0; i < signers.length; i++) {
      hex_key = signers[i].pubKey.toString('hex'); // check if payload sig matches one passed in this param.
    ...
    if (!flag) {
      throw new _errors.ValidationError('Missing signer signing set');
    }
  }
...
const decode_transaction = buffer => {
  ...
  const input_buffer = buffer; // ensure the at the magic is correctly configured
  ...
  const version = (header_first & 0xE0) >> 5; // const charge_unit_flag = Boolean((header_first & 0x08) >> 3)
  ...
  if (version !== VERSION) {
    throw new _errors.ValidationError('Unable to parse transaction from stream, incompatible version');
  } // discard the reserved byte


  buffer = buffer.slice(1);
  const tx = new _transaction.Transaction(); // decode the address from the buffer
  ...
  tx.charge_rate(charge_rate); //  assert not charge_unit_flag, "Currently the charge unit field is not supported"
  ...
  if (contract_type != NO_CONTRACT) {
    ...
    if (!wildcard) {
      ...
      if (!extended_shard_mask_flag) {
        ...
        shard_mask = _bitvector.BitVector.from_bytes(decoded_bytes, bit_size);
      } else {
        const bit_length = 1 << (contract_header_int & 0x3F) + 3;
        const byte_length = Math.floor(bit_length / 8);
        (0, _assert.default)(bit_length % 8 == 0); //this should be enforced as part of the spec

        shard_mask = _bitvector.BitVector.from_bytes(buffer.slice(0, byte_length), bit_length);
    ...
    if (contract_type == SMART_CONTRACT || contract_type == SYNERGETIC) {
      ...
      throw new _errors.RunTimeError('Unhandled contract type');
    }
  ...
  tx.counter(new _bn.BN(buffer.slice(0, 8)));
  ...
  public_keys.forEach(ident => {
    ...
    identity = new _identity2.Identity(ident);
