...
var _bufferReverse = _interopRequireDefault(require("buffer-reverse"));
...
class BitVector {
  ...
  __bytes__() {
    return (0, _bufferReverse.default)(this._buffer);
  ...
  instance_bytes() {
    if (this._buffer) {
      // TODO: Improve logic
      return Buffer.from(this._buffer.toString('hex').match(/.{2}/g).reverse().join(''), 'hex');
    }
  ...
  static from_bytes(data, bit_size) {
    ...
    (0, _assert.default)(min_size <= bit_size <= max_size);
    let bits = new BitVector();
    bits._size = bit_size;
    bits._byte_size = Math.floor((bit_size + 7) / 8); // TODO: Improve logic

    bits._buffer = Buffer.from(data.toString('hex').match(/.{2}/g).reverse().join(''), 'hex');
    return bits;
  }
  ...
  set(bit, value) {
    (0, _assert.default)(0 <= Number(value) <= 1);
    let byte_index = Math.floor(bit / 8);
    let bit_index = bit & 0x7;
