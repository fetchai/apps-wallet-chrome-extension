...
var aesjs = _interopRequireWildcard(require("aes-js"));
 ...
 * @class
 */
class Entity extends _identity.Identity {
  /**
  ...
  constructor(private_key_bytes) {
    ...
    if (private_key_bytes) {
      if (secp256k1.privateKeyVerify(private_key_bytes)) {
        ...
        throw new _errors.ValidationError('Unable to load private key from input');
      }
    } else {
      let privKey;
      let pubKey;
      ...
      do {
        privKey = (0, _crypto.randomBytes)(32);
  ...
  static load(fp, password) {
    if (!Entity._strong_password(password)) {
      throw new _errors.ValidationError('Please enter strong password of 14 length which contains number(0-9), alphabetic character[(a-z), (A-Z)] and one special character.');
    }

    let obj = JSON.parse(_fs.default.readFileSync(fp, 'utf8'));
  ...
  static async _from_json_object(obj, password) {
    const private_key = await Entity._decrypt(password, Buffer.from(obj.password_salt, 'base64'), Buffer.from(obj.privateKey, 'base64'), obj.key_length, Buffer.from(obj.init_vector, 'base64'));
  ...
  async _encrypt(password, data) {
    ...
    const salt = (0, _crypto.randomBytes)(16);
    const promisified_pbkdf2 = (0, _util.promisify)(_crypto.pbkdf2);
    let hashed_pass;

    try {
      hashed_pass = await promisified_pbkdf2(password, salt, 2000000, 32, 'sha256');
    } catch (err) {
      throw new _errors.RunTimeError('Encryption failed');
    } // Random initialization vector


    const iv = (0, _crypto.randomBytes)(16); // Encrypt data using AES
    // https://www.npmjs.com/package/aes-js#cbc---cipher-block-chaining-recommended

    const aes = new aesjs.ModeOfOperation.cbc(hashed_pass, iv); // Pad data to multiple of 16
    ...
    while (data.length) {
      encrypted = Buffer.concat([encrypted, Buffer.from(aes.encrypt(data.slice(0, 16)))]);
      data = data.slice(16);
    }
  ...
  static async _decrypt(password, salt, data, n, iv) {
    ...
    try {
      ...
      throw new _errors.RunTimeError('Decryption failed');
    ...
    while (data.length) {
      decrypted = Buffer.concat([decrypted, Buffer.from(aes.decrypt(data.slice(0, 16)))]);
      data = data.slice(16);
    }

    const decrypted_data = decrypted.slice(0, n); // Return original data
  ...
  static _strong_password(password) {
    if (password.length < 14) {
      console.error('Please enter a password at least 14 characters long');
      return false;
    }

    if (password.match('[a-z]+') === null) {
      console.error('Password must contain at least one lower case character');
      return false;
    }

    if (password.match('[A-Z]+') === null) {
      console.error('Password must contain at least one upper case character');
      return false;
    }
