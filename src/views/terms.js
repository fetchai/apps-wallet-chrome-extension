...
<b>Do not use this application for high-value use:  </b> it is for holding and working with small amounts for utility use on Fetch.ai’s networks.
</li>  <br></br> <li>
<b>This software is in constant development:  </b> please ensure you’re using the latest version, and report all bugs to us on Fetch.ai’s developer Slack.
<br></br></li>
</ul>
By accepting these terms and conditions, you are deemed to have accepted this. It will work with the Fetch.ai Mainnet, Testnet and a locally deployed network.
            It is open-source, and you can examine the source-code for yourself at TODOWRITEGITHUBREPOLINK. For more information about Fetch.ai, please see  <a href=" https://fetch.ai" target="_blank" rel="noopener noreferrer"> https://fetch.ai</a> for legal information, see<a target="_blank" rel="noopener noreferrer" href="https://fetch.ai/legals/">https://fetch.ai/legals/</a>

          </div>
          {this.state.next?[<button  key = {1}  className={"terms-back-button"} onClick = {goTo.bind(null, Initial)}>Back</button>, <button key = {2} className={"terms-accept-button"} onClick = {this.next}>Accept</button>]
