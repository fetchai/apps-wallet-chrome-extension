...
import {
    ...
    DOLLAR_PRICE_URI, EXTENSION,
    NETWORK_NAME,
    TRANSACTION_HISTORY_CHECK_INTERVAL_MS, TRANSITION_DURATION_MS
} from "../constants";
                ...
                <div className="OverlayMainInner">
                    ...
                    >
                        <div className="balance_container">
                            <img className='plus' alt="fetch circular logo" src={getAssetURI("fetch_circular_icon.svg")}/><br></br>
                            {this.state.balance !== null ?
                                <span className="fet-balance">{this.state.balance} FET</span> : ""} <br></br>
                    ...
                    {(this.state.history !== null && Object.keys(this.state.history).length > 0) ?
                        ...
                        >
                            ...
                            <div>
                                ...
                                <div className="history_item"><span
                                    className="history_left_value">{format(this.state.history[0].digest, 13)}</span><span
                                    className="history_right_value">-80</span><br></br>
                                    <span
                                        className="history_left_value light">{this.state.history[0].status}</span><span
                                        className="history_right_value light">{toLocaleDateString(this.state.history[0].created_date)}</span>
                                </div>

                                {Object.keys(this.state.history).length > 1 ? <div className="history_item"><span
                                    className="history_left_value">{format(this.state.history[1].digest, 13)}</span><span
                                    className="history_right_value">-80</span><br></br>
                                    <span
                                        className="history_left_value light">{this.state.history[1].status}</span><span
                                        className="history_right_value light">{toLocaleDateString(this.state.history[1].created_date)}</span>
                                </div> : ""}
                            </div>
